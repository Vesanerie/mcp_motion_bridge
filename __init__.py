...
def _build_prompt(context, mesh, cameras, videos, props):
    ...
    return (
        ...
        "  # Precompute inverses once — do NOT recompute inside the frame loop\n"
        ...
        "       q = Quaternion(r.as_quat()[[3,0,1,2]])  # scipy → Blender wxyz\n"
        "     NEVER do manual axis-angle math.\n"
        "  c) Coordinate conversion (MediaPipe output, applied AFTER step b):\n"
        "       q_bl = Quaternion((q.w, -q.x, q.z, -q.y))\n"
        "     If using HMR2/SMPL (Y-up source): q_bl = Quaternion((q.w, q.x, q.z, -q.y))\n"
        "  d) Rest-offset composition — REQUIRED for EVERY bone, even if it looks correct\n"
        "     without it. Skipping this step WILL produce dislocated shoulders and twisted\n"
        "     limbs on any rig whose rest pose is not a perfect T-pose:\n"
        "       final_rot = rest_inv[bone_name] @ q_bl @ rest_offsets[bone_name]\n"
        ...
        "  f) For pelvis only: also apply location from smpl_trans with coord conversion\n"
        "     and keyframe_insert('location', frame=f)\n\n"
